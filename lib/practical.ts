...
const practicalBySlug: Record<string, Partial<IdeaPractical>> = {
  ...
  'wheelie-bin-space-station': {
    ...
    materials: ['removable card panels', 'foil stars', 'cardboard podium front', 'slogan card', 'tape', 'string'],
    steps: ['Make decorations on card panels first so the bin itself stays clean.', 'Tape stars and a ringed planet to the panels.', 'Make a cardboard podium front or lectern-style placard.', 'Add one bold slogan panel.', 'Rest any lightweight display pieces on the bin only while supervised and safe.', 'Remove everything before collection or bad weather.'],
    wording: ['Count Binface Wheelie Bin Podium.', 'This bin backs Binface.', 'A modest podium for mighty recycling.', 'Collection day still applies.'],
    upgrades: ['Add a cardboard lectern lip.', 'Make a matching mission badge for the lid.', 'Stage a tiny press conference beside it.'],
    sensible: ['Do not stop the lid opening or the bin being moved.', 'Keep the bin off pavements and roads.', 'Remove loose pieces if wind picks up.'],
